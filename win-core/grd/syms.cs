...
namespace grd
{
	public class syms
	{
		...
		public void reshape(int columns, int rows)
		{
			sess.maps[index].reshape(columns, rows);
			sess.code.Append($"sess.maps[{index}].reshape({columns}, {rows});");
		}
		...
		public static syms operator *(syms a, syms b)
		{
			...
			sess.code.Append($"sess.ans=sess.maps[{a.index}]*sess.maps[{b.index}];");
			return tempsyms;
		}
		...
		public void set()
		{
			sess.code.Append($"sess.maps[{index}]).set({sess.ans});");
		}
	}
}
